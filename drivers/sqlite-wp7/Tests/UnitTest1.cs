...
namespace Vici.CoolStorage.SqliteWP7.Tests
{
    [TestClass]
    public class XXTest
    {
 ...
 /*           if (File.Exists(path))
            ...
            */
            CSConfig.SetDB(new CSDataProviderSqliteWP7(@"uri=file://" + path));
...
@"CREATE TABLE tblCoolData (
    ...
    [TestClass]
    public class CommonTests : SilverlightTest
    {
...
/*
        ...
        public void ManyToMany()
        {
            ...
            for (int i = 0; i < numIterations; i++)
            {
                ...
                Assert.AreEqual(2L, cust1.SalesPeople.GetScalar("*", CSAggregate.Count));
                Assert.AreEqual(1L, cust2.SalesPeople.GetScalar("*", CSAggregate.Count));
                Assert.AreEqual(1L, cust3.SalesPeople.GetScalar("*", CSAggregate.Count));
        ...
        public void ComplexFilters()
        {
            ...
            Assert.AreEqual(2, new CSList<OrderItem>("Order.Customer.Name = 'blabla'").Count,"orders with customer blabla not correct");
            Assert.AreEqual(2, new CSList<OrderItem>("len(Order.Customer.Name) = 6").Count,"orders with customer name length of 6 not correct");
            ...
            Assert.AreEqual(1, new OrderCollection("max(OrderItems.Price) = 200").Count,"num orders with maxprice=200 not correct");
            Assert.AreEqual(1, new OrderCollection("sum(OrderItems.Price) = 245").Count,"num orders with total=245 not correct");
            Assert.AreEqual(2, new CSList<Order>("count(OrderItems) = 2").Count,"num orders with 2 items not correct");
            Assert.AreEqual(2, new CSList<Order>("has(OrderItems)").Count,"num orders with orderitems not correct");
            Assert.AreEqual(1, Customer.List("len(Name)=4").Count,"num customers with name length 4 not correct");
            ...
            var orders1 = new OrderCollection("count(OrderItems where Price > 100.0) = 1").ToList();
            var allorderes = new OrderCollection().ToList();

            Assert.AreEqual(1, new OrderCollection("count(OrderItems where Price > 100.0) = 1").Count, "count(orderitems where price>100) not correct");
        ...
        //            int intValue = 5;
        //            int? nIntValue = 6;
        //            SalesPersonType salesPersonTypeValue = SalesPersonType.Internal;
        //            SalesPersonType? nSalesPersonTypeValue = SalesPersonType.External;
        //            short shortValue = 7;
        //            short? nShortValue = 8;
        //
        //            Assert.AreEqual(5, CSHelper.ConvertType(intValue, typeof(int)));
        //            Assert.AreEqual(6, CSHelper.ConvertType(nIntValue, typeof(int)));
        //            Assert.AreEqual(5, CSHelper.ConvertType(intValue, typeof(int?)));
        //            Assert.AreEqual(6, CSHelper.ConvertType(nIntValue, typeof(int?)));
        //            Assert.AreEqual(7, CSHelper.ConvertType(shortValue, typeof(int?)));
        //            Assert.AreEqual(8, CSHelper.ConvertType(nShortValue, typeof(int?)));
        //            Assert.AreEqual(5, CSHelper.ConvertType(intValue, typeof(short)));
        //            Assert.AreEqual(6, CSHelper.ConvertType(nIntValue, typeof(short)));
        //            Assert.AreEqual(5, CSHelper.ConvertType(intValue, typeof(short?)));
        //            Assert.AreEqual(6, CSHelper.ConvertType(nIntValue, typeof(short?)));
        //            Assert.AreEqual(0, CSHelper.ConvertType(salesPersonTypeValue, typeof(int)));
        //            Assert.AreEqual(1, CSHelper.ConvertType(nSalesPersonTypeValue, typeof(int)));
        //            Assert.AreEqual(0, CSHelper.ConvertType(salesPersonTypeValue, typeof(short)));
        //            Assert.AreEqual(1, CSHelper.ConvertType(nSalesPersonTypeValue, typeof(short)));
        //            Assert.AreEqual(SalesPersonType.External, CSHelper.ConvertType(1, typeof(SalesPersonType)));
        //            Assert.AreEqual(SalesPersonType.External, CSHelper.ConvertType((short)1, typeof(SalesPersonType)));
        //            Assert.AreEqual(SalesPersonType.External, CSHelper.ConvertType((byte)1, typeof(SalesPersonType)));
        //            Assert.AreEqual(SalesPersonType.External, CSHelper.ConvertType((long)1, typeof(SalesPersonType)));
        //            Assert.AreEqual(SalesPersonType.Internal, CSHelper.ConvertType(0, typeof(SalesPersonType?)));
        //            Assert.AreEqual(SalesPersonType.Internal, CSHelper.ConvertType(SalesPersonType.Internal, typeof(SalesPersonType?)));
        //
        //            Assert.IsNull(CSHelper.ConvertType(null, typeof(int?)));
        //            Assert.IsNull(CSHelper.ConvertType(null, typeof(SalesPersonType?)));
        //
        //            Assert.AreEqual(1000, CSHelper.ConvertType(null, typeof(int), 1000));
        //            Assert.IsNull(CSHelper.ConvertType(null, typeof(int?), 1000));
        //        }
        //
        [TestMethod]
        public void Paging()
    ...
    public partial class Order : CSObject<Order, int>
    {
        ...
        [ManyToOne]
        [Prefetch]
        public SalesPerson SalesPerson { get { return (SalesPerson)GetField("SalesPerson"); } set { SetField("SalesPerson", value); } }
        [ManyToOne]
        [Prefetch]
        public Customer Customer { get { return (Customer)GetField("Customer"); } set { SetField("Customer", value); } }
        [OneToMany]
        public CSList<OrderItem> OrderItems { get { return (CSList<OrderItem>)GetField("OrderItems"); } }
    }
    ...
    public partial class OrderItem : CSObject<OrderItem, int>
    {
        ...
        [DefaultSort]
        public double Price { get { return (double)GetField("Price"); } set { SetField("Price", value); } }
        public string Description { get { return (string)GetField("Description"); } set { SetField("Description", value); } }

        [ManyToOne]
        public Order Order { get { return (Order)GetField("Order"); } set { SetField("Order", value); } }
    }
